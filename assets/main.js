$(document).ready(function () {
    ...
    function doSearch(pattern, scope, noHistory) {
        ...
        if (hasPattern) {
            ...
            searchBox[0].scrollIntoView({behavior: 'smooth'}); // TODO
        }

        // Adjust result display
        if (cnt > 0) {
            $('#result-count').html(cnt);
            resultPane.toggleClass('hide', false);
        } else {
            resultPane.toggleClass('hide', true);
        }
        noResultsPane.toggleClass('hide', !hasPattern || cnt > 0);
    ...
    function searchCurHash() {
        if (window.location.hash.length > 1) {
            ...
            doSearch(queryParams[0], queryParams.includes('abbr') ? 'abbr' : 'all', true);
        }
    }
    ...
    for (var i = 0; i <=3; i++) {
        ...
        exList.append('<a class="abbr-term mx-1" href="#' + encodeURIComponent(abbr) + '&abbr">' + abbr + '</a>')
    }
